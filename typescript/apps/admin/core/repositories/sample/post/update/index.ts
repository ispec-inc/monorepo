...
export interface ISamplePostUpdateRepository {
  ...
  patch(id: number, payload: ISampleUpdatePayloadModel): Promise<void>
}
...
export class SamplePostUpdateRepositoryImpl implements ISamplePostUpdateRepository {
  ...
  async patch(id: number, payload: ISampleUpdatePayloadModel): Promise<void> {
    await this.endpoint.run(id, payload.toObject()).catch((err) => { throw err })
  }
